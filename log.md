...
## 【页面音乐栏目 · 2026-09-02新增，日常运行不要改动】

index.html 顶部（进度条下方、筛选栏上方）新增了一个 `.music-card` 栏目，嵌入网易云音乐播放器（歌单「【中島美雪】全曲集」id=57217843，355首，用户是网易云会员）。这是用户手动要求加的固定栏目，**日常每天的例行更新不需要动它**，除非用户明确要求换歌单/加歌手/调整样式。
